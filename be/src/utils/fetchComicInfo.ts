...
export function parseComicHtmlPage(html: string, url: string): Comic {
    ...
    if (url.includes("blogtruyenmoi.com")) {
        result = ParseBlogTruyenMoiHtmlPage($);
    } else if (url.includes("nettruyen")) {
        result = ParseNetTruyenHtmlPage($);
    }
...
export function ParseBlogTruyenMoiHtmlPage($: cheerio.CheerioAPI): Comic {
    const result: Comic = {}
    // Fetch description
    ...
    if (description.length && description.find('p').length) {
        description.find('p').each((_, child) => {
            ...
            if (childText.startsWith('Tên khác:')) {
                ...
                result.source = {};
                sourceLink.each((_, link) => {
                    result.source![$(link).text().trim()] = $(link).attr('href') || '';
                })
...
export function ParseNetTruyenHtmlPage($: cheerio.CheerioAPI): Comic {
    const result: Comic = {}
